...
public class SystemClassEx {
    public static void main(String[] args) throws InterruptedException {
        ...
        int cnt = 0, i;
        int randVal = (int)(Math.random() * 1000) + 1;  // 1 ~ 1,000 사이의 임의의 값
        for (i = 0; i < data.length; i++) {
            data[i] = i + 1;
        }
        long start = System.currentTimeMillis();    // 1970년 1월 1일 0시 0분부터 경과 시간을 밀리초 단위로 표시
        for (i = 0; i < data.length; i++) {
            cnt++;
            sleep(1);
            if ( randVal == data[i]) {
